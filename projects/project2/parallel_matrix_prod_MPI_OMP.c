...
/* Function that executes the local matrix product using OpenMP for multi-threaded execution and stores result in local portion my_matrix_C. */
void matrix_multiplication(double** my_matrix_A, double** matrix_B, double*** my_matrix_C, int my_rows_A, int NUM_COLS_A, int NUM_COLS_B){
	int i, j, k;				// Counter variables for triple for loop below.
	omp_set_dynamic(1);			// Allowing dynamic change of number of threads.
	omp_set_num_threads(4);		// Setting the number of threads (per process).
